...
class Customer(db.Model):
    ...
    def update_balance(self):
        ...
        # Calculate total receivables (including additional amounts from linked invoices)
        total_receivables = sum(
            receivable.amount + receivable.additional_amount 
            for receivable in self.receivables
        )
        
        # Update balance (positive means customer owes money, negative means surplus)
        self.balance = total_invoices + total_receivables - total_payments
        return self.balance
...
def product(id):
    ...
    if request.method == 'PUT':
        ...
        product.stock_locations = data.get('stock_locations', '')
...
def invoice(id):
    ...
    if request.method == 'GET':
        ...
        try:
            db.session.commit()
            return jsonify({'success': True, 'redirect': url_for('new_invoice')})
        except Exception as e:
            db.session.rollback()
            return jsonify({'success': False, 'error': str(e)})
    ...
    try:
        ...
        if file.filename.endswith('.xlsx'):
            ...
            for row in sheet.iter_rows(min_row=2):  # Skip header row
                if all(cell.value is not None for cell in row[:5]):  # Ensure required fields are present
                    try:
                        product = {
            ...
            for row_idx in range(1, sheet.nrows):  # Skip header row
                ...
                if all(cell.value for cell in row[:5]):  # Ensure required fields are present
                    try:
                        product = {
                            'item_code': str(row[0].value).strip(),
                            'description': str(row[1].value).strip(),
        ...
        return jsonify({'message': f'Successfully imported {len(products)} products'}), 200
    except Exception as e:
